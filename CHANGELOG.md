...
- **App**: Add RAW image format support via `rawler` crate. Supports CR2, CR3, NEF, ARW, RAF, ORF, RW2, PEF, DNG and many other formats. Includes bilinear demosaicing with automatic white balance and 16-bit output to preserve maximum dynamic range. RAW orientation metadata is also applied. (Native only, disabled for WASM)
