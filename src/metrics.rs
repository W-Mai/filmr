...
impl FilmMetrics {
    pub fn analyze(img: &RgbImage) -> Self {
        ...
        let lab_std = [
            ((sq_sum_lab[0] / count) - (lab_mean[0] * lab_mean[0]))
                .sqrt()
                .max(0.0),
            ((sq_sum_lab[1] / count) - (lab_mean[1] * lab_mean[1]))
                .sqrt()
                .max(0.0),
            ((sq_sum_lab[2] / count) - (lab_mean[2] * lab_mean[2]))
                .sqrt()
                .max(0.0),
        ];
        ...
        let rg_ratio = if mean_rgb[1] > 0.0 {
            mean_rgb[0] / mean_rgb[1]
        } else {
            0.0
        };
        let bg_ratio = if mean_rgb[1] > 0.0 {
...
fn calculate_lbp(img: &RgbImage) -> [f32; 10] {
    ...
    for y in 1..h - 1 {
        for x in 1..w - 1 {
            ...
            if get_lum(img, x - 1, y - 1) >= c {
                code |= 1;
            }
            if get_lum(img, x, y - 1) >= c {
                code |= 2;
            }
            if get_lum(img, x + 1, y - 1) >= c {
                code |= 4;
            }
            if get_lum(img, x + 1, y) >= c {
                code |= 8;
            }
            if get_lum(img, x + 1, y + 1) >= c {
                code |= 16;
