...
fn run_pipeline_trace(r: u8, g: u8, b: u8, exposure_time: f32) {
    ...
    println!(
        "Spectral Exposure (after Norm): R={:.4}, G={:.4}, B={:.4}",
        r_in, g_in, b_in
    );
    ...
    let t_eff = if exposure_time > 1.0 {
        exposure_time.powf(reciprocity)
    } else {
        exposure_time
    };
    ...
    println!(
        "Total Exposure (E=I*t): R={:.4}, G={:.4}, B={:.4}",
        r_exp, g_exp, b_exp
    );
    println!(
        "Log10 Exposure: R={:.4}, G={:.4}, B={:.4}",
        r_exp.log10(),
        g_exp.log10(),
        b_exp.log10()
    );
    ...
    println!(
        "Mixed Density: R={:.4}, G={:.4}, B={:.4}",
        d_r_mix, d_g_mix, d_b_mix
    );
